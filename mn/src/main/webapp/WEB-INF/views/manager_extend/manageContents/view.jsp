				...
				<div id="collapseTwo" class="collapse show"
					...
					<div class="bg-white py-2 collapse-inner rounded">
						...
						<a class="collapse-item active"
							href="/mn/manageContents/ContentsList.do">컨텐츠 목록</a>
					...
					<div class="row">
						...
						<div class="col-lg-6">
							...
							<div class="card shadow mb-4">
								<form class="user" id="myform" action="modifyContent.do"
									method="post" enctype="multipart/form-data">
									<div class="card-header py-2">
										<div class="form-group">
											<h6 class="m-0 font-weight-bold text-primary"
												style="margin-bottom: 0.5rem !important">컨텐츠 타이틀</h6>
											<input class="form-control form-control-user" type="text"
												name="con_title" value="${content.con_title}"
												placeholder="타이틀을 입력해주세요." />
										</div>


										<input type="checkbox" name="con_state" value="n"
											<c:if test="${content.con_state eq 'n'}">checked</c:if>
											style="margin-left: 0.5rem" />숨기기
									</div>
									<div class="card-body">
										<h6 class="m-0 font-weight-bold text-primary"
											style="margin-bottom: 0.5rem !important">카테고리</h6>

										<select id="con_cate"
											<c:if test="${list.size() > 0}"> name="con_cate" </c:if>>
											<c:forEach items="${list}" var="category">
												<option
													<c:if test="${category == content.con_cate}">selected</c:if>>${category}</option>
											</c:forEach>
											<option value="direct">직접입력</option>
										</select> <input type="text" id="direct"
											<c:choose><c:when test="${list.size() > 0}">style="display:none;"</c:when><c:otherwise> name="con_cate"</c:otherwise></c:choose>>

										<h6 class="m-0 font-weight-bold text-primary"
											style="margin: 0.5rem 0 !important">이미지</h6>
										<c:choose>
										...
										<h6 class="m-0 font-weight-bold text-primary"
											style="margin: 0.5rem 0 !important">내용</h6>
										<textarea name="con_content" rows="20" cols="77"
											placeholder="내용을 입력해주세요.">${content.con_content}</textarea>
										<%--  
